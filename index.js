...
function letMeLogIn() {
    ...
    const noteTab = $('li[data-name="Notes"]').dataset.selected = 'false'
    const accTab = $('li[data-name="Account"]').dataset.selected = 'true'
    const html = `
        <div class="img"><img src="./icons/user.svg" alt="not found"></div>
        <div class="form">
        <p>Login To Cuntinue!</p>
            <div class="email-area">
                <label for="email">Email</label>
                <input type="email" name="email" id="email" placeholder="Email@site.com">
            </div>
        ...
        <div class="buttons">
            <button>Forget Password?</button><button>Login</button>
        </div>
        <div class="signup-button">
            Dont Have An Account? <button class="signup-btn">Signup</button>!
        </div>
   
    `
    const loginPage = CreateElm('div', 'login-page')
    loginPage.insertAdjacentHTML('afterbegin', html)
    document.body.append(loginPage)
    const signupButton = $('.signup-btn')
    console.log(typeof signupButton)
    try {
        signupButton.addEventListener('click', letMeSignUp)
    } catch (error) {
        console.error
        signupButton[0].addEventListener('click', letMeSignUp)
    }
}
...
function letMeSignUp() {
    ...
    <div class="img"><img src="./icons/user.svg" alt="not found"></div>
        ...
        <div class="login-button">
            Already Have An Account? <button class="login-btn">Login</button>!
        </div>
    `
    const signPage = CreateElm('div', 'signup-page')
    signPage.insertAdjacentHTML('afterbegin', html)
    document.body.append(signPage)
    const loginButton = $('.login-btn')
    console.log(typeof loginButton)
    try {
        loginButton.addEventListener('click', letMeLogIn)
    } catch (error) {
        loginButton[0].addEventListener('click', letMeLogIn)
    }
